...
?>
<p>
    <button id="w13" class = "btn btn-info" onclick="js:getCleanedticks()" datatoggle="tooltip" title="Use the checkbox column to select all the houses that have been cleaned. All houses are assumed cleaned by default.">Cleaned (ticked)</button>
    <button id="w14" class = "btn btn-danger" onclick="js:getMissedticks()">Missed (ticked)</button>
    <button id="w15" class = "btn btn-danger" onclick="js:getNotcleanedticks()" datatoggle="tooltip" title="Use the checkbox column to select all the houses that have not been cleaned. All houses are assumed cleaned by default.">Not Cleaned (ticked)</button>
    <?php if (Yii::$app->user->can('Update Daily Job Sheet')) { ?>
    <button id="w10" class = "btn btn-success" onclick="js:getPaidticks()">Paid (ticked)</button>
    <button id="w11" class = "btn btn-danger" onclick="js:getUnpaidticks()">Unpaid (ticked)</button>
    <Hr style = "border-top: 3px double #8c8b8b">   
    <button id="w23" class = "btn btn-danger" onclick="js:getAddpretopaid()">Add pre payment (ticked) to Paid</button>  
    <Hr style = "border-top: 3px double #8c8b8b">   
    <button id="w22" class = "btn btn-danger" onclick="js:getTransferticks()">Transfer advance payments (ticked) to future pre-payment: </button> 
    <?= 
    Html::dropDownList('transadv','', ArrayHelper::map(Salesorderheader::find()->where(['>','clean_date',$clean_date_this])->andWhere(['status'=>$status])->orderBy('status')->all(),'sales_order_id','clean_date','status'),['prompt' => '--- select ---','id'=>'w61','class'=>'btn btn-danger','style'=>'width: 200px']) ?>
    
        
    <?php } ?>
    
    <button id="w33" class = "btn btn-danger dropdown-toggle" type="button" data-toggle="dropdown" >Create Future Clean<span class="caret"></span></button>
                <ul class="dropdown-menu">
                    <li><button id="w35" class = "btn btn-danger" onclick="js:getCopyitbytodaysdatesalesorderdetail()">Today's date</button></li>  
                    <li><button id="w36" class = "btn btn-danger" onclick="js:getCopyitbyfrequencysalesorderdetail()">+1 Month</button></li>    
                </ul>
    <br>
     <?php if (Yii::$app->user->can('Update Daily Job Sheet')) { ?>
    <div>
    <Hr style = "border-top: 3px double #8c8b8b">   
    <button id="w21" class = "btn btn-danger" onclick="js:getGocardlesspayticks()" disabled="disabled" datatoggle="tooltip" title="This feature is currently disabled: Customers can be sent a direct debit variable mandate to consent to each time you need payment from them. ">Gocardless One-off (ticked)</button>
         <?= Html::a('Add House', ['product/index'], ['class' => 'btn btn-warning','datatoggle'=>'tooltip', 'title'=> '"This button will take you to Houses to create one. You will then be able to transfer the house through to the list of houses that are part of the daily clean.']) ?>
         <?= Html::a('Back', ['salesorderheader/index'], ['class' => 'btn btn-success']) ?>
        <Hr style = "border-top: 3px double #8c8b8b">
        <button id="w16" class = "btn btn-success" onclick="js:getOwingticks()" disabled="disabled" datatoggle ="tooltip" title="Different message types can be sent to your customer using Twilio, a service that requires a subscription">SMS-(ticked)</button>
        <div>
           <br>
           <?= Html::label('Message: ') ?>
            <br>
           <?= Html::dropDownList('sdmessage','', ArrayHelper::map(Messaging::find()->all(),'id','message'),['prompt' => '--- select ---','id'=>'w33','class'=>'btn btn-success','disabled'=>'disabled' ,'style'=>'width: 200px']) ?>
        </div>
        <Hr style = "border-top: 3px double #8c8b8b">
    </div>
    <?php } ?>
    ...
    'expandOneOnly' => true,
           ...
           'visible'=> !Yii::$app->user->can('Update Daily Job Sheet') ? false : true,
            ...
            [
                ...
                //'options' => ['title'=>['title', 'data-toggle'=>'tooltip']],
...
echo kartik\grid\GridView::widget([
    ...
    'condensed' => false,
    'toolbar' => [
          ['content'=>Html::a('<i></i>',['salesorderdetail/index', 'id' => Yii::$app->session['sales_order_id']],['data-pjax'=>0,'class'=> 'btn-default','title'=>'Reset Grid'])
         ],
    ],
    'responsive' => true,
    'export'=>false,
