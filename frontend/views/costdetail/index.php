    ...
    'expandOneOnly' => true,
            ...
            [
                ...
                'inputType' => kartik\editable\Editable::INPUT_SPIN,
                    'options' => [
                        'pluginOptions' => ['min' => 0.00, 'max' =>1000.00],                        
                    ]
                ],               
            ],
            [
            ...
            [
                    ...
                    'buttons' => [
                            'link' => function ($url, $data,$key) {
                             ...
                             ->from('works_costdetail')
                             ->where(['and','cost_id='.$data->cost_id,'paid=0'])
                             //->andWhere('cleaned="Cleaned"')
                             ->andWhere('cost_detail_id<'.$data->cost_detail_id)
                             ->all(); 
                             $subtotal = 0.00;
                             $i = 0;
                            ...
                            //return $subtotal;
                            if ($subtotal > 0) return Html::a($subtotal."(x".$i.")",  $url);
                    ...
                    'urlCreator' => function ($action, $data, $key, $index) {
                         if ($action === 'link') {
                            ...
                            foreach ($rows as $key => $value)
                            {
                                $url=Url::toRoute(['costdetail/view', 'id' => $rows[$key]['cost_detail_id']]);
                            }
                            Url::remember();
                            return $url;
                        }
