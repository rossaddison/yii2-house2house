...
function getCreategocardlesscustomer (){
    ...
    $.post({ type: "GET",
        ...
        success: alert("The following Gocardless customers were created: Remember to refresh the screen to get the Url's. " + keys)
    });  
...
function getOwingticks (){
    var keys = $('#w1').yiiGridView('getSelectedRows'); 
    var f = document.getElementById("w33");
    //the sales order id value of the date selected in dropdownlist
    var value = f.options[f.selectedIndex].text;
    $.post({ type: "GET",
             url: '/salesorderdetail/owingticked/',
        dataType: "json",
        data: {keylist: keys,sdmessage: value},
        success: $.pjax.reload({container:'#kv-unique-id-1'})        
    });
}
