...
class TaxController extends Controller
{
    ...
    public function actionCreate()
    {
        if (!\Yii::$app->user->can('Create Tax')) {
            throw new \yii\web\ForbiddenHttpException('You do not have permission to create a tax code.');
        }
    ...
    public function actionUpdate($id)
    {
        ...
        if (!\Yii::$app->user->can('Update Tax')) {
            throw new \yii\web\ForbiddenHttpException('You do not have permission to update a tax code.');
        }
    ...
    public function actionDelete($id)
    {
        if (!\Yii::$app->user->can('Delete Tax')) {
            throw new \yii\web\ForbiddenHttpException('You do not have permission to delete a tax code.');
        }
        try{
        $this->findModel($id)->delete();
        return $this->redirect(['index']);
        } catch(IntegrityException $e) {
              throw new \yii\web\HttpException(404, 'First delete Daily cleans or costs that this tax code has been linked to then you will be able to delete this tax code.');
        }
    }
